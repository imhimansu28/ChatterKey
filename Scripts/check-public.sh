...
failed=0
# Report matching filenames only; never echo potential credentials into build logs.
...
if grep -RIlIE \
  --exclude='check-public.sh' --exclude-dir=build --exclude-dir=.gradle --exclude-dir=signing \
...
for path in .build dist .env AGENTS.md apps/android/signing '*.jks' '*.keystore' '*.p12' '*.apk' '*.aab'; do
